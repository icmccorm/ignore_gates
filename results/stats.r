...
better_aux_mem <- by_mem_usage %>% filter(max != control & max != control.noelim & max != control.sideeffects)
...
better_aux_cpu$max_diff <- pmax(better_aux_cpu$percent_diff_from_control, better_aux_cpu$percent_diff_from_control_sideeffects, better_aux_cpu$percent_diff_from_control_noelim)
...
control_compare <- select(better_aux_cpu, benchmark, control.noelim, control.sideeffects, control)
control_compare$best_control <- colnames(control_compare)[apply(select(control_compare, control.noelim, control.sideeffects, control), 1, which.min)+1]
